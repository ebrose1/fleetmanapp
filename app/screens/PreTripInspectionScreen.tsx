...
import {
  validateTirePressure,
  validateTreadDepth,
} from '../utils/validation';

export const PreTripInspectionScreen: React.FC<{
  navigation: any;
  route: any;
}> = ({ navigation, route }) => {
  ...
  const detectWarnings = (): string[] => {
    ...
    const pressures = [
      { label: 'Left Front', value: inspectionData.tiresPsiLf },
      { label: 'Left Rear', value: inspectionData.tiresPsiLr },
      { label: 'Right Front', value: inspectionData.tiresPsiRf },
      { label: 'Right Rear', value: inspectionData.tiresPsiRr },
    ];

    pressures.forEach(({ label, value }) => {
      const validation = validateTirePressure(value);
      if (validation.warning) {
        warnings.push(`${label} tire: ${validation.warning}`);
      }
    });

    // Check tread depths
    const treads = [
      { label: 'Front Left', value: inspectionData.tiresTreadFl },
      { label: 'Front Right', value: inspectionData.tiresTreadFr },
      { label: 'Rear Left', value: inspectionData.tiresTreadRl },
      { label: 'Rear Right', value: inspectionData.tiresTreadRr },
    ];

    treads.forEach(({ label, value }) => {
      const validation = validateTreadDepth(value);
      if (validation.warning) {
        warnings.push(`${label} tread: ${validation.warning}`);
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    // Show warning summary if any exist
  ...
  const submitInspection = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        'Your pre-trip inspection has been saved and will sync when connected.',
        [
          {
            text: 'OK',
